...
class Caracteristicas extends Connection
{
    ...
    public function update($id, $newStatus){
        ...
        // Ejecuta la consulta
        try {
            $sql->execute([$newStatus=='true'?1:0, $id]);
            $this->clear();
    ...
    public function updateComputer($computer)
    {
        ...
        $case = $data['gce_case'];
        $procesador = $data['gce_procesador'];
        $grafica = $data['gce_grafica'];
        $ram = $data['gce_ram'];
        $disco_duro = $data['gce_disco_duro'];
        $teclado = $data['gce_teclado'];
        $mouse = $data['gce_mouse'];
        $pantalla = $data['gce_pantalla'];
        $estado = $data['gce_estado'];

        $query = "UPDATE gce_caracteristicas
        SET gce_nombre_equipo='{$nombre}', gce_board='{$board}', gce_case='{$case}', gce_procesador='{$procesador}', gce_grafica='{$grafica}', gce_ram='{$ram}', gce_disco_duro='{$disco_duro}', gce_teclado='{$teclado}', gce_mouse='{$mouse}', gce_pantalla='{$pantalla}', gce_estado='{$estado}' WHERE gce_id={$id}";
        // Prepara la sentencia
